...
public class LevelEditor {
    ...
    private void updateButtons(MouseHandler mouse) {
        ...
        if (clearButton.isClicked(mouse)) {
            clearMap();
        }
    ...
    public void draw(Graphics2D g, Font customFont) {
        ...
        for (int r = 0; r < GRID_ROWS; r++) {
            for (int c = 0; c < GRID_COLS; c++) {
                ...
                if (type > 0) {
                    ...
                    if (customFont != null) {
                        g.setFont(customFont.deriveFont(12f));
                    } else {
        ...
        for (int i = 0; i < 4; i++) {
            if (i + 1 == currentBrickType) {
                ...
                g.drawRect(
                    typeButtons[i].bounds.x - 2, 
                    typeButtons[i].bounds.y - 2,
                    typeButtons[i].bounds.width + 4, 
                    typeButtons[i].bounds.height + 4
                );
            }
    ...
    // 저장된 데이터를 게임에서 사용할 수 있는 Brick 객체 리스트로 변환
    public ArrayList<Brick> getGeneratedBricks() {
        ...
        for (int r = 0; r < GRID_ROWS; r++) {
            for (int c = 0; c < GRID_COLS; c++) {
                ...
                if (type == 1) {
                    list.add(new NormalBrick(x, y, CELL_WIDTH, CELL_HEIGHT));
                } else if (type == 2) {
                    list.add(new MediumBrick(x, y, CELL_WIDTH, CELL_HEIGHT));
                } else if (type == 3) {
                    list.add(new HardBrick(x, y, CELL_WIDTH, CELL_HEIGHT));
                } else if (type == 4) {
                    list.add(new ExplosiveBrick(x, y, CELL_WIDTH, CELL_HEIGHT));
                }
            }
        }
        return list;
