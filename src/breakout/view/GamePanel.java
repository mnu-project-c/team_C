...
public class GamePanel extends JPanel implements Runnable {
    ...
    boolean isUnlocked = (currentSkinIndex == -1) || 
                         achievementManager.isUnlocked("학생회의 자격-" + (currentSkinIndex + 1));

    if (currentSkinIndex != -1 && isUnlocked) {
    ...
    public String getBallSkinName() { 
        if (currentSkinIndex == -1) return "NONE";
        String name = "학생회의 자격- " + (currentSkinIndex + 1);
        
        // 업적 달성 여부를 확인하여 잠금 표시 추가
        if (!achievementManager.isUnlocked("학생회의 자격-" + (currentSkinIndex + 1))) {
            return name + " (🔒 LOCKED)";
        }
        return name;
    }
