...
public class SettingsPanel {
    ...
    private void initUI() {
        ...
        this.previewTextY = paddleY + 90;
        this.previewY = previewTextY + 70;
        this.brickY = previewY + 50;
    ...
    public boolean update(MouseHandler mouse) {
        ...
        if (bgPrevButton.isClicked(mouse)) {
            gamePanel.prevBackground();
        }
        if (bgNextButton.isClicked(mouse)) {
            gamePanel.nextBackground();
        }
        ...
        if (gamePanel.isSoundOn()) {
            soundButton.setText("소리: ON");
        } else {
            soundButton.setText("소리: OFF");
        }

        crtFilterButton.update(mouse);
        if (crtFilterButton.isClicked(mouse)) {
            gamePanel.toggleCRTFilter();
        }
        
        if (gamePanel.isCRTFilterOn()) {
            crtFilterButton.setText("CRT 필터: ON");
        } else {
            crtFilterButton.setText("CRT 필터: OFF");
        }

        ballColorButton.update(mouse);
        if (ballColorButton.isClicked(mouse)) {
            gamePanel.cycleBallColor();
        }
        ballColorButton.setText("공 색상: " + gamePanel.getBallColorName());

        ballSkinButton.update(mouse);
        if (ballSkinButton.isClicked(mouse)) {
            gamePanel.cycleBallSkin();
        }
        ballSkinButton.setText("공 스킨: " + gamePanel.getBallSkinName());

        paddleColorButton.update(mouse);
        if (paddleColorButton.isClicked(mouse)) {
            gamePanel.cyclePaddleColor();
        }
        paddleColorButton.setText("패들 색상: " + gamePanel.getPaddleColorName());

        paddleShapeButton.update(mouse);
        if (paddleShapeButton.isClicked(mouse)) {
            gamePanel.cyclePaddleShape();
        }
        paddleShapeButton.setText("패들 모양: " + gamePanel.getPaddleShapeName());

        brickColorButton.update(mouse);
        if (brickColorButton.isClicked(mouse)) {
            gamePanel.cycleBrickColor();
        }
